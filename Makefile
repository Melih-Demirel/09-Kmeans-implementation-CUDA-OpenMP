...
# Passed argument via make call (only 1)
...
build: mainOmp.cpp rng.cpp
	$(CXX) $(FLAGS) -o $(OMP_EX) -fopenmp mainOmp.cpp rng.cpp

buildCuda: mainCuda.cu rng.cpp
	$(NVCC) $(FLAGS) -o $(CUDA_EX) mainCuda.cu rng.cpp $(CUDA_FLAGS)

runCuda:
	$(eval DATA=$(DATA3))
	./$(CUDA_EX) --blocks $(ARGUMENT) --threads $(THREADS_CUDA) --input $(DATA_PATH)/$(DATA).csv --output $(OUTPUT_CUDA_PATH)/output$(DATA).csv --seed $(SEED) --k $(K) --repetitions $(REPETITIONS)


runCuda10metingenMetBlock:
	$(eval DATA=$(DATA3))
	for i in $$(seq 1 10); do \
		./$(CUDA_EX) --blocks $(ARGUMENT) --threads $(THREADS_CUDA) --input $(DATA_PATH)/$(DATA).csv --output $(OUTPUT_CUDA_PATH)/output$(DATA).csv --seed $(SEED) --k $(K) --repetitions $(REPETITIONS); \
	done

run: mainOmp.cpp rng.cpp
	$(eval DATA=$(DATA3))
	./$(OMP_EX) --threads $(THREADS) --input $(DATA_PATH)/$(DATA).csv --output $(OUTPUT_OMP_PATH)/output$(DATA).csv --seed 1953139 --k 20 --repetitions 40

remOutput:
	rm kmeans.slurm.o* kmeans.slurm.e*
...
test: mainOmp.cpp rng.cpp
	...
	$(CXX) $(FLAGS) -o $(OMP_EX) -fopenmp mainOmp.cpp rng.cpp
	$(eval DATA=$(DATA3))
	./$(SERIAL_EX) --input $(DATA_PATH)/$(DATA).csv --output $(OUTPUT_SERIAL_PATH)/output$(DATA).csv --seed 1337 --k 10 --repetitions 20
	./$(OMP_EX) --threads 1 --input $(DATA_PATH)/$(DATA).csv --output $(OUTPUT_OMP_PATH)/output$(DATA).csv --seed 1337 --k 10 --repetitions 20
