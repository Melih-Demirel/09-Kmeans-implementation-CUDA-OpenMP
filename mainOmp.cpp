           ...
           const std::string &centroidDebugFileName, const std::string &clusterDebugFileName)
{
    ...
    for (int r = 0; r < repetitions; r++)
    {
        ...
        while (changed)
        {
            ...
            #pragma omp parallel for shedule(static) num_threads(numThreads) reduction(+:distanceSquaredSum)
            ...
            #pragma omp parallel for shedule(static) num_threads(numThreads)
            for (size_t j = 0; j < numClusters; j++)
            {
