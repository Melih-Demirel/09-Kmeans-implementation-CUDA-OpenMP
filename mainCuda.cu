...
struct DeviceData
{
    thrust::device_vector<double> d_allData;
                                          ...
                                          size_t numCols, size_t numRows, int numClusters)
{
    ...
    for (size_t index = tid; index < numRows; index += blockDim.x * gridDim.x)
    {
        double closestDistance = __DBL_MAX__;
        size_t closestCentroidIndex = 0;

        // Load data for this block into shared memory
        for (size_t i = localId; i < numCols * numClusters; i += blockDim.x)
        {
            sharedMem[i] = centroids[i];
        }
        __syncthreads();
        ...
        for (size_t centroidIndex = 0; centroidIndex < numClusters; centroidIndex++)
        {
            ...
            for (size_t dimensionIndex = 0; dimensionIndex < numCols; ++dimensionIndex)
            {
                double diff = allData[index * numCols + dimensionIndex] - sharedMem[centroidIndex * numCols + dimensionIndex];
                distance += diff * diff;
            }
...
void findClosestCentroidCUDA(DeviceData &deviceData, thrust::device_vector<bool> &changes, double &distanceSquaredSum, size_t numCols, size_t numRows, int numClusters, int numThreads, int numBlocks)
{
    ...
    // Calculate loopSize = how many calculations 1 Cuda Thread will do.
    findClosestCentroidKernel<<<numBlocks, numThreads, numClusters * numCols * sizeof(double)>>>(
        thrust::raw_pointer_cast(deviceData.d_allData.data()),
        thrust::raw_pointer_cast(deviceData.d_centroids.data()),
        thrust::raw_pointer_cast(deviceData.d_clusters.data()),
        thrust::raw_pointer_cast(changes.data()),
        d_distanceSquaredSum,
        numCols, numRows, numClusters);
...
__global__ void accumulateCentroidsAndCountPointsKernel(const double *allData, double *newCentroids, int *clusterNumPoints,
                                                        const int *clusters, size_t numCols, size_t numPoints, size_t numClusters)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int index = tid; index < numPoints; index += stride)
    {
...
__global__ void normalizeCentroidsKernel(double *newCentroids, const int *clusterNumPoints, size_t numCols, size_t numClusters)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int index = tid; index < numClusters; index += stride)
    {
        for (size_t dimensionIndex = 0; dimensionIndex < numCols; ++dimensionIndex)
        {
...
// This phase concist of 2 kernel calls.
// 1: accumulate: we add up all the points with same centroid in newCentroids vector with correct index. While doing this we keep count of how many there are too.
// 2: normalize: here we divide by the count of points in each newCentroids vector.

void calculateNewCentroidCUDA(DeviceData &deviceData, double *centroids, size_t numCols, size_t numRows, int numClusters, int numThreads, int numBlocks)
{
    // Allocate device memory for data
    thrust::device_vector<double> d_newCentroids(numCols * numClusters, 0);
    thrust::device_vector<int> d_clusterNumPoints(numClusters, 0);

    accumulateCentroidsAndCountPointsKernel<<<numBlocks, numThreads>>>(
        thrust::raw_pointer_cast(deviceData.d_allData.data()),
    ...
    normalizeCentroidsKernel<<<numBlocks, numThreads>>>(
        thrust::raw_pointer_cast(d_newCentroids.data()),
        thrust::raw_pointer_cast(d_clusterNumPoints.data()),
        numCols, numClusters);
           ...
           const std::string &centroidDebugFileName, const std::string &clusterDebugFileName)
{
    ...
    for (int r = 0; r < repetitions; r++)
    {
        ...
        deviceData.d_allData = thrust::device_vector<double>(allData.data(), allData.data() + numRows * numCols);

        while (changed)
        {
            deviceData.d_centroids = thrust::device_vector<double>(centroids.data(), centroids.data() + numClusters * numCols);
            deviceData.d_clusters = thrust::device_vector<int>(clusters.data(), clusters.data() + numRows);
            // Calculate
            {
                changed = false;
                distanceSquaredSum = 0.0;
                thrust::fill(changes.begin(), changes.end(), false);
                findClosestCentroidCUDA(deviceData, changes, distanceSquaredSum, numCols, numRows, numClusters, numThreads, numBlocks);
                thrust::copy(deviceData.d_clusters.begin(), deviceData.d_clusters.end(), clusters.begin());
                changed = thrust::count(changes.begin(), changes.end(), true) > 0;
            }
            {
                calculateNewCentroidCUDA(deviceData, centroids.data(), numCols, numRows, numClusters, numThreads, numBlocks);
            }
